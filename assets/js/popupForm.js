$(function () {
  $(document).on("click", ".addTweetBtn", function () {
    ...
    $.post(
      "http://localhost/interdisciplinar/core/ajax/tweetForm.php",
      function (data) {
  ...
  $(document).on("submit", "#popupForm", function (e) {
    ...
    $.ajax({
      url: "http://localhost/interdisciplinar/core/ajax/addTweet.php",
      type: "POST",
      data: formData,
      success: function (data) {
        ...
        if (result["error"]) {
          $(
            '<div class="error-banner"><div class="error-banner-inner"><p id="errorMsg">' +
              result.error +
              "</p></div></div>"
